...
h2. Rails 3 Asset Pipeline

Follow the Split Theme Storage describe here: https://github.com/lucasefe/themes_for_rails/wiki/Assets-Pipeline-Integration

On config/application.rb:
<pre>
ThemesForRails.available_themes.each do |theme_dir|
  config.assets.paths << theme_dir
end
</pre>

On config/production.rb:
<pre>
config.assets.precompile += ['[theme]/stylesheets/theme.css'] # Precompile asset themes
config.assets.digest_themes_for_rails = true # Enable digest on Themes for Rails assets
</pre>
